 ...
 * { cardId, rating: again|hard|good|easy|1-4, answer?, feedback?, durationMs?, deck?, tag?, mode? }
 * Reschedules the card and returns the next question for the same deck/tag filter.
 */
export const POST = route(async ({ req, user }) => {
  const { cardId, deck, tag, mode, ...grade } = await readJson(req);
  ...
  return { result, next: nextCard(user.id, { deck, tag, mode }) };
});
